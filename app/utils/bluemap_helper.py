...
Contains helper functions to synchronize waypoint data into the static
BlueMap .conf files for each dimension. We only manage a "warp" marker-set.
After updating each dimension's .conf, we call /bluemap reload via RCON.
"""
...
def sync_waypoints_bluemap(all_waypoints):
    ...
    - For each dimension:
      1) Load the .conf file from BLUEMAP_MAPS_PATH + DIMENSION_TO_BLUEMAP_CONF[dimension]
      2) Ensure "marker-sets" and "warp" set exist
      3) Replace all markers in "warp" set with the up-to-date list for that dimension
      4) Write the file back
    - Finally, call bluemap_reload() to pick up changes
    """
    dimension_map = {}
    ...
    # For each dimension, update the relevant .conf
    for dim, wps in dimension_map.items():
        conf_filename = DIMENSION_TO_BLUEMAP_CONF.get(dim)
        if not conf_filename:
            logging.warning(f"No BlueMap .conf file known for dimension '{dim}'. Skipping.")
            continue

        conf_path = os.path.join(BLUEMAP_MAPS_PATH, conf_filename)
        if not os.path.isfile(conf_path):
            logging.warning(f"BlueMap config file not found at: {conf_path}. Skipping dimension '{dim}'.")
            continue
        ...
        # Update lines with our warp markers
        ...
        logging.info(f"Synchronized {len(wps)} waypoint(s) to {conf_filename} for dimension '{dim}'.")

    # After updating all dimensions, reload BlueMap
    bluemap_reload()
